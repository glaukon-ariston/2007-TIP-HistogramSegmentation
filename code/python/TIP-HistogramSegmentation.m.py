...
def pool_adjacent_violators(h, nonDecreasing):
    '''
    https://en.wikipedia.org/wiki/Isotonic_regression
    Compute the isotonic regression of an histogram h::np.array.
...
def max_entropy(h, a, b, e, nonDecreasing):
    ...
    for i in range(0, L):
        for j in range(i, L):
            ...
            if v > max_entropy:
                max_entropy = v
    max_entropy = (max_entropy - threshold) * N
    return max_entropy
...
def fine_to_coarse_histogram_segmentation(H, e):
    ...
    # Loop on all optimas
    for k in range(0, K-3+1):
        # decide if we want to test the increasing or decreasing hypothesis on
        # [idxs(k), idxs(k+3)]
        nonDecreasing = 1 if (begins_with_min and k % 2 == 1) or (not begins_with_min and k % 2 == 0)
            else 0
    ...
    while maxEntropies.size > 0 and valmin < 0:
        ...
        idxs = np.concatenate([idxs[0:kmin], idxs[kmin+3:]])       
        ...
        for j in range(max(kmin-2,0), min(kmin,maxEntropies.size)):
            ...
            nonDecreasing = 1 if (begins_with_min and j % 2 == 1) or (not begins_with_min and j % 2 == 0)
                else 0 
    ...
    idxs = idxs(0::2) if begins_with_min 
        else idxs(1::2);     
    return idxs
